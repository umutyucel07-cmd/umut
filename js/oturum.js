...
(function () {
  ...
  window.UYOturum = {
    ...
    kodTalep: function (ad, tel) {
      ...
      var kayit = { t: new Date().toISOString(), ad: String(ad).trim(), son4: son4, eslesti: !!m, durum: m ? 'bekliyor' : 'reddedildi' };
      ...
      var istek = fetch(uc, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ad: adN, tel: rakam, kaynak: location.hostname }) }).then(function (r) {
        if (!r.ok) throw new Error('uc-hata');
        kayit.durum = 'gonderildi';
        return { durum: 'gonderildi', mesaj: 'Kodunuz kayıtlı WhatsApp numaranıza gönderilmiştir; birkaç dakika içinde ulaşır.' };
      });
      ...
      return Promise.race([istek, zamanAsimi]).catch(function () { return null; }).then(function (r) {
        if (r) return r;
        return { durum: 'kuyruk', mesaj: 'Talebiniz alınmıştır. Kodunuz kayıtlı WhatsApp numaranıza en geç mesai saatleri içinde iletilecektir.' };
      });
    },
  };
})();
