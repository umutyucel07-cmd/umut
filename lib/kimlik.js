...
/**
 * Kod normalizasyonu. Hem YENİ biçimi (UY-XXXX-XXXX) hem ESKİ biçimi
 * (UY-0000) kabul eder; eski biçim dağıtılmış kodlar için geriye dönük
 * uyumluluktur, yeni kod ÜRETİLMEZ.
 */
export function kodNormalize(v) {
  let t = String(v || '')
    .toLocaleUpperCase('tr-TR')
    .replace(/[\s\-–—._]+/g, '');
  if (/^\d{4}$/.test(t)) t = KOD_ONEK + t;                    // "4182" → "UY4182"
  if (!t.startsWith(KOD_ONEK)) return '';
  const govde = t.slice(KOD_ONEK.length);
  if (/^\d{4}$/.test(govde)) return `${KOD_ONEK}-${govde}`;    // eski biçim
...
/**
 * Yeni erişim kodu üretir: UY-XXXX-XXXX.
 *
 * NEDEN DÖRT HANE YETMİYOR: UY-0000..UY-9999 = 10.000 olasılık. 472 müvekkile
...
/**
 * IP başına pencere sayacı. KV eventual consistent olduğu için bu SIKI bir
 * sınır değil, kaba bir frendir. Asıl koruma kod uzayının büyüklüğüdür.
 */
